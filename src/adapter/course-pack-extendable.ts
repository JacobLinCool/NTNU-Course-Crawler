...
export class CoursePackExtAdapter extends Adapter {
    static id = "course-pack-ext";

    public async process(courses: CourseInfo[]): Promise<any> {
        log_progress("Adapting to Extendable Course Pack ...");
        ...
        for (const program of programs.values()) {
            const p = { id: program, name: program };
        ...
        for (const course of courses) {
            const c: PackedCourse = {
                id: `${course.year}-${course.term}-${course.serial}`,
                name: course.name,
        ...
        log_progress("Adapted to Extendable Course Pack");
        log_progress.done();
